// https://github.com/ttk592/spline/blob/master/examples/bench.cpp
...
{                                                            \
   ...
   for(int i=bench::TK;  i<=bench::SPLINE; i++){             \
...
public:
    ...
    double bench_convert(method m, int num_ops)
    {
        ...
        return (m==TK||m==SPLINE) ? m_Y2[n] : m_AY2[n];
    }
...
int main(int argc, char** argv)
{
    ...
    printf("\ntk vs ALGLIB accuracy: max difference = %.2e, l2-norm difference = %.2e\n",
           linf, l2);

    b.compare(l2,linf,bench::SPLINE, bench::ALGLIB, loops);
    printf("this vs ALGLIB accuracy: max difference = %.2e, l2-norm difference = %.2e\n",
           linf, l2);
